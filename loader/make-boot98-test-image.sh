...
output="${1:-$loader_dir/../build/boot98/boot98-stage1-test.img}"
...
make -C "$loader_dir" disk-ipl.bin boot98-stage1.bin boot98-chain-test.bin

size="$(stat -c%s "$loader_dir/boot98-stage1.bin")"
...
dd if="$loader_dir/disk-ipl.bin" of="$output" bs=512 count=1 conv=notrunc status=none
...
dd if="$loader_dir/boot98-chain-test.bin" of="$output" bs=512 seek=2176 count=1 conv=notrunc status=none
