...
build_noct()
{
	...
	case "$action" in
		...
		verify)
			...
			make -C "$repo/bootloader" noct-m8-verify "$@"
			;;
		softfloat-test)
		...
		lifecycle-test)
			...
			;;
		jit-test)
			make -C "$repo/bootloader" noct-m6-generated-opcode-check "$@"
			;;
		status)
...
Commands:
  ...
  lifecycle-test run the M8 lifecycle, argument, and native-API host test
  jit-test      audit native code generated by the i386 JIT
  softfloat-test run the M5 arithmetic/conversion/math known vectors
  verify        verify M4-M8 lifecycle, native APIs, soft-float, and i386 JIT
