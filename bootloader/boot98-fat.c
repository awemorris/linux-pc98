...
static int parse_bpb(const struct boot98_volume *volume,
		    ...
		    enum boot98_fat_type required_type)
{
	...
	return parse_bpb(volume, &candidate) && candidate.type == required_type &&
		    ...
		    enum boot98_fat_type required_type)
{
	...
	return parse_bpb(&filesystem->volume, fat) &&
	       fat->type == required_type &&
			  ...
			  uint32_t end_of_chain)
{
	...
	while (length) {
		uint32_t lba = fat->data_start +
		               (cluster - 2) * fat->sectors_per_cluster + skip;
		uint32_t chunk = 512 - within;
		const uint8_t *input = boot98_fat_read_sector(filesystem, lba);
			      ...
			      boot98_fat_next_cluster_t next_cluster)
{
	...
	*absolute_lba = filesystem->volume.start_lba + fat->data_start +
			(uint32_t)(fat_file->first_cluster - 2) *
