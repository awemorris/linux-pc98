...
def fat16_layout(total_sectors, reserved, sectors_per_cluster=4,
    ...
    while True:
        ...
        needed = math.ceil((clusters + 2) * 2 / SECTOR_SIZE)
...
def write_fat16(image, start_lba, total_sectors, kernel_path):
    pbr = bytearray(SECTOR_SIZE)
    ...
    pbr[0:3] = b"\xEB\x3C\x90"
    pbr[3:11] = b"MIRAI98 "
    struct.pack_into("<H", pbr, 0x0B, SECTOR_SIZE)
    pbr[0x0D] = spc
    struct.pack_into("<H", pbr, 0x0E, reserved)
    pbr[0x10] = fats
    struct.pack_into("<H", pbr, 0x11, root_entries)
    struct.pack_into("<H", pbr, 0x13, 0)
    pbr[0x15] = 0xF8
    ...
    struct.pack_into("<I", pbr, 0x20, total_sectors)
    pbr[0x24] = 0x80
    pbr[0x26] = 0x29
    struct.pack_into("<I", pbr, 0x27, 0x3938394C)
    pbr[0x2B:0x36] = b"MIRAI98BOOT"
    pbr[0x36:0x3E] = b"FAT16   "
    # A deliberately ordinary, non-booting FAT16 PBR.  DOS can replace it
    # during installation; Linux booting is handled by LBA 0 and LBA 2.
    pbr[0x3E:0x43] = b"\xFA\xF4\xEB\xFD\x90"
    ...
    write_zeros(image, base, total_sectors * SECTOR_SIZE)
    image.seek(base)
    image.write(pbr)

    fat_offset = base + reserved * SECTOR_SIZE
...
def create(args):
    ...
    with open(args.output, "w+b") as image:
        ...
        fat_info = write_fat16(image, p1_start, p1_sectors, args.kernel)
        make_ext4(
...
def update_kernel(args):
    with open(args.image, "r+b") as image:
        ...
        fat_info = write_fat16(image, start_lba, sectors, args.kernel)
    print(
