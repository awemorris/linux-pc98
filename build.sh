...
build_noct()
{
	...
	case "$action" in
		...
		verify)
			...
			make -C "$repo/bootloader" noct-m7-verify "$@"
			;;
		softfloat-test)
...
Commands:
  ...
  lifecycle-test run the M7 interpreter/JIT lifecycle and argument test
  jit-test      audit native code generated by the i386 JIT
  softfloat-test run the M5 arithmetic/conversion/math known vectors
  verify        verify M4-M7 lifecycle, arguments, soft-float, and i386 JIT
