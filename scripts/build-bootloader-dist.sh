...
install -m 0644 "$repo/bootloader/LS.NCT" "$stage/LS.NCT"
install -m 0644 "$repo/bootloader/CP.NCT" "$stage/CP.NCT"
...
(
	...
	zip -X -9 -q "$output.part.$$" \
		...
		CP.NCT HELLO.NCT INST.EXE IO.SYS IPL-LBA0.IMG IPL-LBA2.IMG \
		IPL-PART.IMG LS.NCT \
