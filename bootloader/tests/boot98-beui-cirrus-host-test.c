/*
 * Host test for the Boots Core-Graph / Cirrus BeUI backend
...
main(void)
{
	...
	struct boot98_beui_rect rect = { 7, 9, 3, 2 };
	...
	io.crtc[0x27] = 0xa0;
	io.misc = 1;
	boot98_beui_pc98_cirrus_default(&backend);
	backend.io_context = &io;
	backend.port_in8 = mock_in8;
	backend.port_out8 = mock_out8;
	backend.framebuffer = framebuffer;
	assert(boot98_beui_pc98_cirrus_make_hal(&hal, &backend));
	...
	assert(io.wab[3] == 0);
	assert(io.sleep == 0);
	puts("Boots Core-Graph/Cirrus BeUI host test: PASS");
	return 0;
}
