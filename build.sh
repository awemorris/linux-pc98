...
build_noct()
{
	...
	case "$action" in
		...
		verify)
			...
			make -C "$repo/bootloader" noct-m6-verify "$@"
			;;
		softfloat-test)
...
Commands:
  ...
  lifecycle-test run the M6 interpreter/JIT lifecycle parity test
  jit-test      audit native code generated by the i386 JIT
  softfloat-test run the M5 arithmetic/conversion/math known vectors
  verify        verify M4-M6 lifecycle, soft-float, and i386 JIT opcodes
